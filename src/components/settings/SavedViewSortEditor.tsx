...
export function SavedViewSortEditor({ viewName, sorts, onChange }: { viewName: string; sorts: VpsListSortCondition[]; onChange: (sorts: VpsListSortCondition[]) => void }) {
  ...
  const byKey = new Map(sorts.map((sort) => [sort.key, sort]));
  const drag = useDragReorder(sortKeys, (keys) => onChange(keys.map((key) => byKey.get(key)!).filter(Boolean)), (key) => VPS_LIST_SORT_LABELS[key]);
  ...
  return <section className="studio-sort-editor" aria-label={`${viewName} 的列表排序`}>
    <header><div><h3>比较顺序</h3><p>先按第一项排序；值相同时，再比较下一项。可拖拽调整优先级。</p></div><button type="button" disabled={sorts.length >= VPS_LIST_SORT_KEYS.length} onClick={() => {
    ...
    <ol>{sorts.map((condition, index) => <li key={condition.key} {...drag.getItemProps(condition.key)}>
      <button type="button" className="studio-drag-handle" {...drag.getHandleProps(condition.key)}><GripVertical size={16} /></button>
      <span className="studio-sort-number" aria-hidden="true">{index + 1}</span>
      <label>比较字段<select value={condition.key} aria-label={`${viewName} 的第 ${index + 1} 个排序字段`} onChange={(event) => { const key = event.target.value as VpsListSortKey; update(index, { key, direction: recommendedVpsListSortDirection(key) }); }}>{VPS_LIST_SORT_KEYS.map((key) => <option key={key} value={key} disabled={sorts.some((item, position) => position !== index && item.key === key)}>{VPS_LIST_SORT_LABELS[key]}</option>)}</select></label>
    ...
    <span className="sr-only" role="status">{drag.announcement || announcement}</span>
  </section>;
}
