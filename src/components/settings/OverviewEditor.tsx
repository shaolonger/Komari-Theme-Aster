...
export function OverviewEditor({ overview, groupsVisible, offlineLast, onOverviewChange, onGroupsChange, onOfflineChange, groups, loading, onMove }: {
  ...
  return <div className="studio-overview-editor"><div className="studio-setting-group">
    ...
    {groups.length === 0 ? <p role="status">{loading ? '正在加载分组…' : '暂无分组，为节点设置分组后即可调整。'}</p> : <ol>{groups.map((group, index) => <li key={group}><span className="studio-group-number">{index + 1}</span><strong>{group}</strong><button type="button" disabled={index === 0} aria-label={`上移 ${group}`} onClick={() => onMove(index, -1)}><ArrowUp size={16}/></button><button type="button" disabled={index === groups.length - 1} aria-label={`下移 ${group}`} onClick={() => onMove(index, 1)}><ArrowDown size={16}/></button></li>)}</ol>}
  </section></div>;
}
